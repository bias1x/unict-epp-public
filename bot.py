...
def get_anteprima(url, headers):
    try:
        ...
        if res.status_code != 200: return "Dettagli nel link."
    ...
    for b in BACHECHE:
        try:
            ...
            for l in links:
                ...
                # FILTRO LASER: ignoriamo il tasto rosso dello screenshot
                if "avvisi del corso di laurea" in testo.lower():
                    continue
                
                if any(x in href for x in ['/comunicazioni/', '/avvisi/', '/news/', '/avviso/']):
                    if len(testo) > 15:
                        blacklist = ['/home', '/elenco-news', 'avvisi docente', 'didattica']
